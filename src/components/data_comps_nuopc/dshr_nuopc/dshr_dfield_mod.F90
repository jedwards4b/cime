...
!===============================================================================
  ...
  subroutine dshr_dfield_add_strmfld(dfields, sdat, strm_fld, strm_ptr, logunit, masterproc)
    ...
    do ns = 1, sdat%nstreams
       ...
       call ESMF_FieldBundleGet(sdat%streamdata(ns)%fldbun_model, fieldNameList=lfieldnamelist, rc=rc)
       if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=u_FILE_u)) &
            call ESMF_Finalize(endflag=ESMF_END_ABORT)
  ...
  subroutine dshr_dfield_add_2d(dfields, sdat, state_fld, strm_flds, state, &
    ...
    do nf = 1, nflds
       ...
       do ns = 1, sdat%nstreams
          ...
          call ESMF_FieldBundleGet(sdat%streamdata(ns)%fldbun_model, fieldName=trim(dfield_new%stream_fldnames(nf)), &
    ...
    do while (associated(dfield))
       ...
       if (associated(dfield%state_data1d)) then
          ...
          if (stream_index /= iunset .and. fldbun_index /= iunset) then
             call dshr_fldbun_getfieldn(sdat%streamdata(stream_index)%fldbun_model, fldbun_index, lfield, rc=rc)
             if (chkerr(rc,__LINE__,u_FILE_u)) return
          ...
          do nf = 1,dfield%stream_nflds
             ...
             if (stream_index < 1 .or. stream_index > sdat%nstreams) then
                call shr_sys_abort("stream_index out of range")
             endif
             if (fldbun_index < 1 .or. fldbun_index > sdat%nstreams) then
                call shr_sys_abort("fldbun_index out of range")
             endif
             call dshr_fldbun_getfieldn(sdat%streamdata(stream_index)%fldbun_model, fldbun_index, lfield, rc=rc)
             if (chkerr(rc,__LINE__,u_FILE_u)) return
             call dshr_field_getfldptr(lfield, fldptr1=data1d, rc=rc)
